...
class RequirementExtractor:
    ...
    def get_normalized_value(requirement_list: List) -> Optional[Dict]:
        ...
        # 构建返回值：始终包含原文，如果有规整后的值也包含
        result = {}
        if original_text is not None and original_text != "":
            result["原文"] = original_text
        
        if normalized_value is not None:
            # 如果规整后的值是字典，合并到结果中
            if isinstance(normalized_value, dict):
                result.update(normalized_value)
            else:
                result["规整后"] = normalized_value
